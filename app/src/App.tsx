...
function AppAuth(){
  ...
  return (
    <>
      ...
      <App containerName={accounts[0].localAccountId} />
    </>
  )
...
function AppWithAuth() {
  ...
  const [isAuth, setIsAuth] = useState(accounts.length > 0);
  ...
  useEffect(() => {
    ...
    async function handleRedirectPromise() {
      ...
      try {
        const res = instance.handleRedirectPromise();
        console.log(res);
        setIsAuth(true);
      } catch (error) {
        console.error(error);
  ...
  if (!isAuth) return (<div>Authenticating...</div>);
